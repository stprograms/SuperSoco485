...
1. Include the `SuperSoco485.h` header, this will provide the `SuperSoco485` instance.
...
## Energy optimization
In case the interfaces are temporarily shut down, it is possible that incomplete data lies in the library buffer. To reset the buffer and start with a clean one when the interfaces are enabled again, the `flush` function can be used.
